...
# 坐标变换矩阵
R = np.array([[1, 0, 0], [0, 0.5, 0.866], [0, -0.866, 0.5]])
t = np.array([[0.036], [0.28], [-0.01]])
    ...
    try:
        while True:
            ...
            if len(camera_xyz_list) == 4:
                # 对矩阵进行排序，从左到右，从上到下
                camera_xyz_list.sort(key=lambda x: x[1])
                camera_xyz_list[:2] = sorted(camera_xyz_list[:2], key=lambda x: x[0])
                camera_xyz_list[2:] = sorted(camera_xyz_list[2:], key=lambda x: x[0])
                # 转换为numpy数组
                camera_xyz_arr = np.array(camera_xyz_list)
                # 坐标变换
                tmp_camera_arr = (R @ camera_xyz_arr.T + t) * 1000
                new_camera_arr = tmp_camera_arr.T
            ...
            with open(filename, 'w') as file_object:
                if (len(camera_xyz_list) == 4):
                    file_object.write(str(new_camera_arr[3][0])[0:5] + " " + str(new_camera_arr[3][1])[0:5] + " " + str(
                        new_camera_arr[3][2])[0:5] + " " + "\n")
                    file_object.write(str(new_camera_arr[1][0])[0:5] + " " + str(new_camera_arr[1][1])[0:5] + " " + str(
                        new_camera_arr[1][2])[0:5] + " " + "\n")
                    file_object.write(str(new_camera_arr[2][0])[0:5] + " " + str(new_camera_arr[2][1])[0:5] + " " + str(
                        new_camera_arr[2][2])[0:5] + " " + "\n")
                    file_object.write(str(new_camera_arr[0][0])[0:5] + " " + str(new_camera_arr[0][1])[0:5] + " " + str(
                        new_camera_arr[0][2])[0:5] + " " + "\n")
