...
// std::fstream fs;
static int count = 0;  // 用来统计工作的次数，到达设定的次数后停止
char buf[2048];  // 接收ToF传感器返回的信息
...
int main()
{
    ...
    if (connectIPC(cmApi, "10.10.56.214", 23234))
    {
        ...
        // 需要另开一个碰撞检测线程，进行实时检测，防止碰撞
        pthread_t pid;
        pthread_create(&pid, NULL, ToF_Senser, NULL);
...
void *ToF_Senser(void *arg)
{
    ...
    while (1)
    {
        ...
        for (int i = 9; i < ret; i = i + 6)
        {
            ...
            {
                std::cout << "Danger! Distance too close, stop working and return to base point." << std::endl;
                pMot.moveTo(base_point);
                pthread_exit(NULL);
            }
        }
        cout << "--------------" << endl;
...
void ToF_Senser_Start()
{
    ...
    while (1)
    {
        ...
        for (int i = 9; i < ret; i = i + 6)
        {
            ...
            float result = temp / 1000.0f;
            cout << result << " ";
            // 检测转盘是否运动到合适位置，什么时候开始第一次工作
            if (result > 490 && result < 510)
            {
